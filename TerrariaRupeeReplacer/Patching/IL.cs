...
namespace TerrariaRupeeReplacer.Patching {
	...
	public struct ILCheck {
		public OpCode OpCode;
		public InstructionCheck Check;
		public ILCheck(OpCode opCode) {
			OpCode = opCode;
			Check = (i, op) => { return true; };
		}
		public ILCheck(OpCode opCode, InstructionCheck check) {
			OpCode = opCode;
			Check = check;
		}
		public ILCheck(OpCode opCode, ParameterDefinition variable) {
			OpCode = opCode;
			Check = (i, op) => { return (op is ParameterDefinition && (ParameterDefinition)op == variable); };
		}
		public ILCheck(OpCode opCode, VariableDefinition variable) {
			OpCode = opCode;
			Check = (i, op) => { return (op is VariableDefinition && (VariableDefinition)op == variable); };
		}
		public ILCheck(OpCode opCode, double value) {
			OpCode = opCode;
			Check = (i, op) => { return (op is double && (double)op == value); };
		}
		public ILCheck(OpCode opCode, float value) {
			OpCode = opCode;
			Check = (i, op) => { return (op is float && (float)op == value); };
		}
		public ILCheck(OpCode opCode, long value) {
			OpCode = opCode;
			Check = (i, op) => { return (op is long && (long)op == value); };
		}
		public ILCheck(OpCode opCode, int value) {
			OpCode = opCode;
			Check = (i, op) => { return (op is int && (int)op == value); };
		}
		public ILCheck(OpCode opCode, byte value) {
			OpCode = opCode;
			Check = (i, op) => { return (op is byte && (byte)op == value); };
		}
		public ILCheck(OpCode opCode, sbyte value) {
			OpCode = opCode;
			Check = (i, op) => { return (op is sbyte && (sbyte)op == value); };
		}
		public ILCheck(OpCode opCode, string value) {
			OpCode = opCode;
			Check = (i, op) => { return (op is string && (string)op == value); };
		}
		public ILCheck(OpCode opCode, FieldReference field) {
			OpCode = opCode;
			Check = (i, op) => { return (op is FieldReference && ((FieldReference)op).FullName == field.FullName); };
		}
		public ILCheck(OpCode opCode, MethodReference method) {
			OpCode = opCode;
			Check = (i, op) => { return (op is MethodReference && ((MethodReference)op).FullName == method.FullName); };
		}
		public ILCheck(OpCode opCode, TypeReference type) {
			OpCode = opCode;
			Check = (i, op) => { return (op is TypeReference && ((TypeReference)op).FullName == type.FullName); };
		}
	}
	...
	public static class IL {
		...
		public static void MethodInsert(MethodDefinition method, int nStartIndex, IEnumerable<Instruction> instructions) {
			var il = method.Body.GetILProcessor();
			
			foreach (var inst in instructions) {
				il.Body.Instructions.Insert(nStartIndex, inst);
				nStartIndex++;
			}
		}
		...
		public static int ScanForInstructionPattern(MethodDefinition m, params ILCheck[] inst) {
		...
		public static int ScanForNthInstructionPattern(MethodDefinition m, int n, int nStartOffset, params ILCheck[] inst) {
			var il = m.Body.GetILProcessor();

			for (var x = nStartOffset; x < il.Body.Instructions.Count - inst.Length; x++) {
				if (il.Body.Instructions[x].OpCode != inst[0].OpCode)
					continue;

				for (var y = 0; y < inst.Length; y++) {
					if (il.Body.Instructions[x + y].OpCode != inst[y].OpCode)
						break;
					if (!inst[y].Check.Invoke(x + y, il.Body.Instructions[x + y].Operand))
						break;
					if (y == inst.Length - 1) {
						if (n == 0)
							return x;
